...
const C = {
  ...
  goldLight: "#E8C56F", coralStart: "#FF8A6B", coralEnd: "#E8527D",
  ink: "#1A1714", inkSoft: "#5A5247", inkMute: "#9A9080",
  border: "#E8DFC8", borderSoft: "#F0E8D4", cream: "#FAF6EB", white: "#FFFFFF",
};

interface Doc { id: string; driveUrl: string; module: { title: string } }
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: 13, fontWeight: 700, color: C.ink, margin: "0 0 2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {doc.module.title}
        </p>
        <p style={{ fontSize: 11, color: C.greenAccent, margin: 0 }}>Ouvrir dans Google Drive ↗</p>
      </div>
    </a>
  );
}
    ...
    <div style={{ maxWidth: 640, margin: "0 auto", padding: "28px 16px 48px", background: C.cream, minHeight: "100vh" }}>
      ...
      <div style={{ position: "relative", overflow: "hidden", borderRadius: 20, background: `linear-gradient(160deg, ${C.greenDeep} 0%, ${C.greenDeeper} 100%)`, padding: "28px 28px 24px", marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.67)", margin: 0 }}>
          {grouped.reduce((s, g) => s + g.docs.length, 0)} document{grouped.reduce((s, g) => s + g.docs.length, 0) !== 1 ? "s" : ""} disponible{grouped.reduce((s, g) => s + g.docs.length, 0) !== 1 ? "s" : ""}
        </p>
      </div>
